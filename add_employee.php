...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO employees (name, main_branch, store_branch, job_position, work_schedule, age, contact_no, status, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssiiss", $name, $main_branch, $store_branch, $job_position, $work_schedule, $age, $contact_no, $status, $imagePath);
    $stmt->execute();
    $stmt->close();

    // Redirect back to tracking with same params
